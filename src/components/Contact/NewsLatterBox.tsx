...
// Removed: import axios, { AxiosInstance } from "axios";

const LoadingOverlay = () => (
    // Note: The z-index must be high enough to cover everything, z-50 is good.
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-75 backdrop-blur-sm transition-opacity duration-300">
        <div className="flex flex-col items-center bg-white dark:bg-gray-800 p-8 rounded-xl shadow-2xl text-center w-80">
            <Loader2 className="h-10 w-10 text-primary animate-spin mb-4" /> {/* Changed color to primary for consistency */}
            <h5 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Subscribing...</h5>
            <p className="text-sm text-gray-600 dark:text-gray-400">
                Please wait while subscribing.
            </p>
        </div>
    </div>
);
const NewsLatterBox = () => {
  ...
  const [isLoading, setIsLoading] = useState(false); // State to control the loading overlay
  // 2. Set mounted to true once on the client after initial render/hydration.
  useEffect(() => {
    setMounted(true);
  }, []);

  // 3. Define a function to get the correct stop color.
  const getStopColor = () => {
    if (!mounted) {
      // Default color for server-side render/initial client render 
      return "#4A6CF7"; 
    }
...
const [email, setEmail] = useState("");
  ...
  const handleNewsletterSubmit =async (e: React.FormEvent) => {
    ...
    try {
      // Assuming a different endpoint for general subscription if needed
      const response = await axios.post(`${API_BASE_URL}/public/subscribe`, { name, email });
      console.log("Subscription successful", response.data);
      alert(response.data);
      // Add success notification logic here
    } catch (error) {
      console.error("Subscription failed", error);
      alert("Subscription failed. Check console for details."); // Added an alert for the error case
  ...
  return (
    <>
      {/* 🛑 THE FIX: Conditionally render the LoadingOverlay here */}
      ...
      <div className="shadow-three dark:bg-gray-dark relative z-10 rounded-xs bg-white p-8 sm:p-11 lg:p-8 xl:p-11">
        ...
        <form onSubmit={handleNewsletterSubmit}>
          ...
          >
            {isLoading ? 'Processing...' : 'Subscribe'} {/* Optional: Change button text */}
          </button>
        ...
        {/* SVG Background Elements - remain unchanged */}
        {/* ... (all your existing SVG code here) ... */}
